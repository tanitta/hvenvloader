...
最も簡単な始め方は、Houdini で `venv > Create Houdini Package` を実行することです。この shelf tool は dialog を開き、保存場所、project name、import package name、Houdini environment variable name、Python requirement、作成する標準 Houdini directory を指定できます。実行すると、Python package layout、`pyproject.toml`、`hpackage.json` が作成されます。
